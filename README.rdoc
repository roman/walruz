...
Actor classes may use several methods to check if the actor can perform the given action on the subject. This are:

- `can?(action, subject)`
  Returns boolean that says if the actor can execute or not the action on the subject, if a block is given, this will be executed when the authorization is true, and a parameters hash from the policy is passed to the block.
...
Examples:
  ...
  # all the code executed bello will be executed when the actor is athorized
...
Sometimes policies can turn really messy, specially when you have a complex business model. The good news is that, normally this complex policies are a composition of more simple policies (eg. UserCanSeePictures). Instead of creating this new classes that replicates the same logic of basic policies, we could merge them together in the following way:

ActorCanSeePictures = Walruz::Utils.andP(UserIsFriend, UserAllowsDisclosureOfPictures)
...
One other thing that the utility methods do for you is leave their track on the returned policy parameters, when you invoke a composite policy, every policy will leave a symbol with the name of the policy underscored and a question mark at the end, that way you can know which policies were successful or not. 
...
Sometimes you would like to have a Policy that strictly depends in other ones, on the previous example `UserAllowsDisclosureOfPictures` could have a dependency that says that only the User allows the disclosure of pictures if and only if there is a friends relationship, so we could re-implement this policy as:
...
Somtimes you would like to execute policies that are not directly related to a subject, but to the association of a subject. Given the example above of the friendship relationship and the disclosure of pictures, sometimes you would like to check if a user can see a picture directly on the picture model.
...
The first parameter of `lift_subject` is the name of the method that returns a new subject, this new subject is then passed through the policy specified on the second parameter and then executes the Policy. Pretty neat eh?

== Returning custom errors

Suppose you want to add an error to the authorization failure to have a more descriptive message, you can do so on the `authorized?` method passing a hash with a :error_message key on the false return. If you use the `can!` method on the actor model, this will become the `Walruz::NotAuthorized` error message.
...
This gem is ready to get going with any Rails 2.x project, you just have to run the generator:
